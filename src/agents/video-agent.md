...
## 役割
**ブログ記事からスライドベースの高品質解説動画**を自動生成する。

### 新ワークフロー（推奨）
```
ブログ記事 → スライド生成 → Marp PDF → 画像変換 → Remotion動画 + 音声
```

- **スライド生成**: Gemini 3 Pro Preview でコンテンツ構成
- **スライド画像**: Gemini 2.5 Flash image でアニメ風イラスト
- **PDF変換**: Marp CLI でPDFエクスポート
- **画像変換**: PDF→個別PNG（各スライド）
- **動画生成**: Remotion でスライド動画
- **音声**: Gemini 2.5 Flash Native Audio でナレーション

### 従来ワークフロー（互換性維持）
記事の全文を読み込み、主要テーマ、統計データ、専門家の見解を抽出し、
**TTS音声ナレーション**と**記事画像**を統合して30秒の概要動画を生成。
...
| 形式 | MP4 (H.264 + AAC) |
| 用途 | YouTube、ブログ埋め込み |
| 音声 | Gemini 2.5 Flash TTS ナレーション |
| **品質評価** | **なし（記事のみ評価）** |
