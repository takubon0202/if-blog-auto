...
|----------|--------------|----------------|----------|
| 情報収集（メイン） | `gemini-3-pro-preview` + Multi-Search 3回 | 月〜土 | 約10秒 |
| 情報収集（深層） | `deep-research-pro-preview-12-2025` | 日曜のみ | 約5分 |
| 記事生成 | `gemini-3-pro-preview` | 全曜日 | 約30秒 |
| 画像生成 | `gemini-2.5-flash-image` | 全曜日 | 約1-2分 |
| スライド生成 | `gemini-3-pro-preview` + `gemini-2.5-flash-image` | 全曜日 | 約3-5分 |
| ナレーション | `gemini-2.5-flash` TTS | 全曜日 | 約30秒 |
| 動画生成 | Remotion 4.0 SlideVideo | 全曜日 | 約5-10分 |
...
| ID | 説明 | 解像度 | 長さ | 用途 |
|----|------|--------|------|------|
| `SlideVideo` | スライドベース動画（推奨） | 1920x1080 | 60秒（可変） | YouTube、解説動画 |
| `SlideVideoShort` | スライドショート | 1080x1920 | 30秒（可変） | Shorts/Reels |
...
品質が95%未満の場合、最大3回まで自動リトライします：

1. スライド数が不足 → スライド数を+2して再生成
2. テキスト量が多すぎる → 簡潔化して再生成
3. 画像生成失敗 → 再生成を試行
